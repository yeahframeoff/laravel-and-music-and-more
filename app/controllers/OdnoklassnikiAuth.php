...
class OdnoklassnikiAuth implements AuthInterface
{
    ...
    {
        Route::get('successOdnoklassnikiAuth', self::success());
    }

    public static function getAuthLink(){
        $full_link = self::AUTH_LINK . '?client_id=' .  self::APP_ID
            . '&response_type=code&redirect_uri=http://target-green.codio.io:3000/successOdnoklassnikiAuth';
        Route::get('successOdnoklassnikiAuth', function(){
            dd('tmp');
        });
        ...
        $full_link = $full_link . '?' . implode('&', $paramsList);
        */
        return $full_link;
    }
